...
def download_csv(address):
    if address not in csv_storage:
        return "Error: No CSV data found for the specified address."

    csv_data = csv_storage[address]

    # Convert CSV data to BytesIO for download
    output = io.BytesIO()
